<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<div class="d-flex justify-content-center">
	<div class="col-7">
		<div class="d-flex justify-content-center">
			<h1 class="display-4 font-weight-bold">리뷰페이지 - 상세</h1>
		</div>
		<div class="mt-2">
			<div class="d-flex justify-content-between p-2 border border-dark">
				<span class="ml-5">별점</span>
				<div class="mr-5">
					<span class="mr-4">작성자</span>
					<span>날짜</span>
				</div>
			</div>
		</div>
		<div class="mt-3 border border-dark min-400">
			<div class="m-2">
			 	내용
			</div>
		</div>
		<div class="d-flex">
			<div class="my-3 border border-dark w-100">
				<div class="m-2">
					<span>댓글</span>
				</div>
			</div>
			<button type="button" class="btn bg-green border border-dark ml-3 my-3 col-2">댓글 쓰기</button>
		</div>
	</div>
</div>